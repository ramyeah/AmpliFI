...
// ─── Chapter row ──────────────────────────────────────────────────────────────
function ChapterRow({ chapter, completedLessons, moduleColor, index }) {
    ...
    <Animated.View style={[crow.container, { opacity, transform: [{ translateY }] }]}>
      ...
      <View style={crow.middle}>
        ...
        <View style={crow.lessonsRow}>
          ...
          <Text style={crow.lessonsText}>{lessonsDone}/{totalLessons} lessons</Text>
        </View>
      </View>
    ...
    <View style={s.container}>
      ...
      >
        ...
        <Animated.View style={[s.statsRow, { opacity: bodyOpacity }]}>
          <StatPill icon="💰" value={totalFincoins}              label="FinCoins" color={mod.color} />
          <StatPill icon="📚" value={`${completedCount}/${totalLessons}`} label="Lessons"  color={mod.color} />
          <StatPill icon="🎯" value={mod.chapters.length}        label="Chapters" color={mod.color} />
        </Animated.View>
        ...
        <Animated.View style={[s.section, { opacity: bodyOpacity }]}>
          ...
          {nextModule ? (
            <View style={[s.nextCard, { borderColor: nextModule.color + '50', backgroundColor: nextModule.colorLight ?? C.lightGray }]}>
            ...
            <View style={[s.nextCard, { borderColor: C.warningDark + '50', backgroundColor: C.warningLight }]}>
              <View style={[s.nextIconCircle, { backgroundColor: C.warningDark }]}>
                <Text style={s.nextIcon}>🎓</Text>
              </View>
              <View style={s.nextText}>
                <Text style={[s.nextModuleLabel, { color: C.warningDark }]}>JOURNEY COMPLETE</Text>
...
const s = StyleSheet.create({
  ...
  backText: { fontFamily: F.semiBold, fontSize: 15, color: 'rgba(255,255,255,0.85)' },
  ...
  moduleCompleteLabel: {
    ...
    color: 'rgba(255,255,255,0.7)', letterSpacing: 2, marginBottom: 4,
  },
  moduleTitle: {
    fontFamily: F.extraBold, fontSize: 26,
    color: C.white, textAlign: 'center', lineHeight: 32,
  },
  moduleDesc: {
    fontFamily: F.regular, fontSize: 13,
    color: 'rgba(255,255,255,0.75)', textAlign: 'center', lineHeight: 19, marginTop: 4,
  },
  ...
  nextText:        { flex: 1 },
  nextModuleLabel: { fontFamily: F.extraBold, fontSize: 10, letterSpacing: 1, marginBottom: 3 },
  nextModuleTitle: { fontFamily: F.extraBold, fontSize: 16, color: C.textPrimary, marginBottom: 3 },
  nextModuleDesc:  { fontFamily: F.regular,   fontSize: 12, color: C.textMuted, lineHeight: 17 },
...
const stat = StyleSheet.create({
  ...
  icon:  { fontSize: 22 },
  value: { fontFamily: F.extraBold, fontSize: 20 },
  label: { fontFamily: F.semiBold,  fontSize: 11, color: C.textMuted },
});
...
const crow = StyleSheet.create({
  ...
  icon:        { fontSize: 22 },
  middle:      { flex: 1, gap: 6 },
  title:       { fontFamily: F.bold,      fontSize: 14, color: C.textPrimary },
  lessonsRow:  { flexDirection: 'row', alignItems: 'center', gap: 4 },
  lessonDot:   { width: 8, height: 8, borderRadius: 4 },
  lessonsText: { fontFamily: F.regular,   fontSize: 11, color: C.textMuted, marginLeft: 4 },
  right:       { alignItems: 'flex-end' },
  fincoins:    { fontFamily: F.extraBold, fontSize: 16 },
