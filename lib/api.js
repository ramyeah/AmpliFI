...
// ── Stage 2: Fin analyses the user's budget after they lock it in ─────────────
// RAG-powered — retrieves Singapore budgeting context from knowledge base.
// Returns a personalised assessment of their Needs/Wants/Savings split.
export const simBudgetFeedback = async ({ userName, income, needsPct, wantsPct, savingsPct, needsAmt, wantsAmt, savingsAmt, ffn }) => {
  const yearsToFI = ffn && savingsAmt > 0
    ? (() => {
        // Rough years to FI: solve FV = PMT * ((1+r)^n - 1) / r >= ffn
        // Simplified: log estimate
        const r = 0.06 / 12;
        if (r === 0) return Math.ceil(ffn / (savingsAmt * 12));
        let months = 0;
        let fv = 0;
        while (fv < ffn && months < 600) { fv = fv * (1 + r) + savingsAmt; months++; }
        return months < 600 ? Math.ceil(months / 12) : null;
      })()
    : null;

  const needsFlag   = needsPct > 60  ? 'high'   : needsPct < 40  ? 'low'   : 'ok';
  const savingsFlag = savingsPct < 10 ? 'low'    : savingsPct >= 20 ? 'good' : 'ok';

  try {
    return await ragAsk(
      '50/30/20 budget rule Singapore needs wants savings student income allocation',
      `You are Fin. ${userName} just locked in their first budget on income of $${income.toLocaleString()}/month:
- Needs: ${needsPct}% = $${needsAmt.toLocaleString()}/month ${needsFlag === 'high' ? '(above the 50% target)' : needsFlag === 'low' ? '(unusually low)' : '(within target)'}
- Wants: ${wantsPct}% = $${wantsAmt.toLocaleString()}/month
- Savings: ${savingsPct}% = $${savingsAmt.toLocaleString()}/month ${savingsFlag === 'low' ? '(below recommended 20%)' : savingsFlag === 'good' ? '(at or above 20% — well done)' : ''}
${ffn ? `- Their FI Number from Stage 1: $${ffn.toLocaleString()}` : ''}
${yearsToFI ? `- At $${savingsAmt.toLocaleString()}/month invested at 6%, they'd reach FI in ~${yearsToFI} years` : savingsAmt === 0 ? '- At $0/month saved, they will never reach their FI Number' : ''}

Write 2-3 sentences of honest, specific feedback on their budget:
${needsFlag === 'high' ? `Their Needs are above 50% — acknowledge this is common in Singapore (rent/transport costs are high) but show the trade-off in concrete terms.` : ''}
${savingsFlag === 'low' ? `Their savings rate is low — be honest about what this means for their FI timeline. Use their actual numbers.` : ''}
${savingsFlag === 'good' ? `Their savings rate is solid — validate it and show the compounding impact using their actual numbers.` : ''}
${yearsToFI ? `Mention the ${yearsToFI}-year FI timeline concretely.` : ''}

Be direct and specific. Use their actual dollar amounts. No generic phrases like "Great job!" or "It's important to note". Sound like a real financial advisor reacting to a real budget.`,
      { name: userName }
    );
  } catch {
    if (savingsAmt === 0) {
      return { response: `$0 to savings means your FI Number stays out of reach indefinitely. Every month you delay costs you compounding — starting with even $${Math.round(income * 0.1).toLocaleString()}/month (10%) would change the picture significantly.` };
    }
    if (savingsPct >= 20) {
      return { response: `${savingsPct}% to savings — $${savingsAmt.toLocaleString()}/month — is a strong start. At 6% annual returns, you'd reach your FI Number in roughly ${yearsToFI ?? 'X'} years if you stay consistent. The 50/30/20 rule exists because this ratio has historically worked.` };
    }
    return { response: `${savingsPct}% to savings gives you $${savingsAmt.toLocaleString()}/month working toward your future. ${needsPct > 55 ? `Your Needs are above 50% — that's common in Singapore given rental and transport costs, but watch that it doesn't creep higher.` : `The 50/30/20 benchmark suggests 20% to savings — you're at ${savingsPct}%, which means your FI timeline is longer than it needs to be.`}` };
  }
};