 ...
 *
 *   redoSection(lessonId, sectionIndex, sectionFincoins)
 *     → called when a user REDOES an already-completed section.
 *     → awards sectionFincoins (the earnable for that section only).
...
/**
 * Called when a user REDOES an already-completed section.
 * Awards sectionFincoins for that section only.
 * Does NOT re-trigger lesson/chapter/module cascade.
 *
 * @param {string} lessonId       e.g. '2-2'
 * @param {number} sectionIndex   0-based index
 * @param {number} sectionFincoins  fincoins value for this specific section
 */
export const redoSection = async (lessonId, sectionIndex, sectionFincoins = 10) => {
  const uid = auth.currentUser?.uid;
  if (!uid) return null;
  ...
  // Award section fincoins only
  await updateDoc(doc(db, 'users', uid), {
    finCoins: increment(sectionFincoins),
    xp:       increment(sectionFincoins),
  });
...
export const resetProgress = async () => {
  ...
  // Also reset finCoins and xp in users/{uid}
  await updateDoc(doc(db, 'users', uid), {
    finCoins: 0,
    xp:       0,
  });
