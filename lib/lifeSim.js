...
export const advanceMonth = async (uid, eventFired = null, decisions = {}) => {
  ...
  // New time gate
  const nextMonthAvailableAt = Date.now() + MONTH_GATE_MS;
  const pending = sim.pendingFinCoins ?? 0;
  ...
  // Flush pending FinCoins to real balance
  if (pending > 0) {
    await updateDoc(doc(db, 'users', uid), {
      finCoins: increment(pending),
      xp:       increment(pending),
    });
    syncSimToStore(pending);
  }
...
// ─── queueFinCoins ───────────────────────────────────────────────────────────
/**
 * Adds FinCoins to the pending queue (flushed on month advance).
 */
export const queueFinCoins = async (uid, amount) => {
  if (!uid) uid = auth.currentUser?.uid;
  if (!uid) return null;
  await updateDoc(simRef(uid), { pendingFinCoins: increment(amount), updatedAt: Date.now() });
  const snap = await getDoc(simRef(uid));
  return snap.exists() ? snap.data() : null;
};
